...
export default function AlgorithmCheck() {
  ...
  const scoreWithXquik = useCallback(async (draft: string) => {
    ...
    try {
      ...
      console.error('AlgorithmCheck: scoreDraft failed', err)
      setAlgoResult(null)
      setAlgoError(err instanceof Error ? err.message : 'Algoritma servisine ulaşılamadı')
    } finally {
          ...
          {hasText ? (
            <>
              ...
              <div className={`card rounded-2xl p-6 ${algoResult ? (algoResult.passed ? getScoreBg(100) : getScoreBg(algoScore)) : 'bg-slate-50 dark:bg-white/3'}`}>
                ...
                {algoError && !algoLoading && (
                  <div
                    role="alert"
                    className="text-xs text-red-600 dark:text-red-400 mt-2 flex items-center gap-2"
                  >
                    <span>⚠ {algoError}</span>
                    <button
                      type="button"
                      onClick={() => scoreWithXquik(text)}
